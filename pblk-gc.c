...
static int pblk_gc_write(struct pblk *pblk)
{
	...
	list_cut_position(&w_list, &gc->w_list, gc->w_list.prev);   //list划分
	gc->w_entries = 0;
	spin_unlock(&gc->w_lock);
...
/*GC工作启动函数，调用pblk_gc_move_valid_secs*/
...
static void pblk_gc_line_prepare_ws(struct work_struct *work)
{
	...
	sec_left = pblk_line_vsc(line);                 //剩余可用的sec
	if (sec_left < 0) {
...
/*gc line总函数，调用gc_line_prepare_ws*/
